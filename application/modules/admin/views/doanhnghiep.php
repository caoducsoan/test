...
<p class="sidebar"><a href="<?php echo site_url('admin/frmvieclam'); ?>">Thêm mới</a> <input type="submit" name="submit" value="Xóa" /></p>
<table width="100%" style="display:block;">
    ...
    ?>
		<tr class="<?php echo $stt%2 ? 'odd' : 'even'; ?>">
			...
			<td><a href="<?php echo site_url('admin/edit_doanhnghiep/'.$item->usc_id); ?>"><?php echo $item->usc_company; ?></a></td>			
			<td align="center"><?php 
			//$ngays	=explode(' ',$item->created_day);
			//$ngay=explode('-',$ngays[0]);
			//$created_day=$ngay[2].'-'.$ngay[1].'-'.$ngay[0];			
			echo date('d/m/Y',$item->usc_create_time); ?></td>			
			<td align="center">						
			  <?php 
			  if($item->usc_active=='1')
			  {					
			  //if(time() > $item->new_create_time){
			  ?>
			  <!--<a class="status" onclick="check_status(<?php echo $item->usc_id; ?>,'new','vieclam')"><img src="images/toolbar/publish_y.png"></a>-->
			  <?php
			  //}else{
			  ?>
			  <a class="status" onclick="check_statusck('user_company','usc_active',0,'usc_id',<?php echo $item->usc_id; ?>)"><img src="images/toolbar/tick.png"></a>
			  <?php //}
			  }
			  else
			  {
			  ?>
			  <a class="status" onclick="check_statusck('user_company','usc_active',1,'usc_id',<?php echo $item->usc_id; ?>)"><img src="images/toolbar/publish_x.png"></a>
			  <?php
			  }
			  ?>
			</td>
            <td align="center">						
			  <?php 
			  if($item->usc_authentic=='1')
			  {
			  ?>
			  
			  <a class="status" onclick="check_statusck('user_company','usc_authentic',0,'usc_id',<?php echo $item->usc_id; ?>)"><img src="images/toolbar/tick.png"></a>
			  <?php 
			  }
			  else
			  {
			  ?>
			  <a class="status" onclick="check_statusck('user_company','usc_authentic',1,'usc_id',<?php echo $item->usc_id; ?>)"><img src="images/toolbar/publish_x.png"></a>
			  <?php
			  }
			  ?>
			</td>  		
			<td><?php 
				echo Getcitybyindex($item->usc_city);
			?></td>
									
			<td align="center"><?php echo $item->usc_id; ?></td>
		</tr>		
    <?php 
    }
    ?>
...
function check_statusck(tblname,field,status,fieldid,id)
{
	$.ajax({
		cache:false,
			type:"POST",  
			url:"http://localhost/timviec1/admin/statusck", 
