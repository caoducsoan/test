...
<table width="100%" style="display:block;">
    ...
    ?>
		<tr class="<?php echo $stt%2 ? 'odd' : 'even'; ?>">
			<td align="center">
			<div id="request-form">
				<input type="checkbox" name="checkbox[]" class="checkbox" value="<?php echo $item->use_id; ?>" />
			</div>
			</td>					
			<td><a href="<?php echo site_url('admin/edit_ungvien/'.$item->use_id); ?>"><?php echo $item->use_first_name; ?></a></td>	
            <td><?php echo $item->use_email; ?></td>		
			<td align="center"><?php 
			//$ngays	=explode(' ',$item->created_day);
			//$ngay=explode('-',$ngays[0]);
			//$created_day=$ngay[2].'-'.$ngay[1].'-'.$ngay[0];			
			echo date('d/m/Y',$item->use_create_time); ?></td>			
			<td align="center">						
			  <?php 
			  echo GetCategory(intval($item->cv_cate_id));
			  ?>
			</td>
            <td align="center">						
			  <?php 
			  if($item->use_authentic=='1')
			  {
			  ?>
			  
			  <a class="status" onclick="check_statusck('user','use_authentic',0,'use_id',<?php echo $item->use_id; ?>)"><img src="images/toolbar/tick.png"></a>
			  <?php 
			  }
			  else
			  {
			  ?>
			  <a class="status" onclick="check_statusck('user','use_authentic',1,'use_id',<?php echo $item->use_id; ?>)"><img src="images/toolbar/publish_x.png"></a>
			  <?php
			  }
			  ?>
			</td>  		
			<td><?php 
				echo Getcitybyindex($item->use_city);
			?></td>
									
			<td align="center"><?php echo $item->use_id; ?></td>
		</tr>		
    <?php 
    }
    ?>
