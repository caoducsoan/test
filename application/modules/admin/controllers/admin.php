...
class admin extends Controller
{
	...
	function custom()
    {
		...
		$query=$this->admin_model->gettbl('tbl_custom_html','');			
		$total_rows = $query->num_rows();
		$this->load->library('pagination');
		...
		$config['last_link'] = '>>';    		
		$this->pagination->initialize($config);
		$data['query']=$this->admin_model->gettbl_limited('tbl_custom_html',$start_row,$per_page);
		$data['pagination']= $this->pagination->create_links();	
		$data['content']='custom';			
		$this->load->view('template',$data);    	   
    }
	...
	function add_custom()
    {
		...
		$this->admin_model->add_tbl('tbl_custom_html',$data,$id);		
    ...
    function vieclam()
    {
        ...
        if(isset($_POST['findkey']) OR isset($_POST['category']) OR isset($_POST['city']) OR isset($_POST['hot'])OR isset($_POST['do'])OR isset($_POST['gap'])OR isset($_POST['cao'])){	
			unset($_SESSION['findkey']);
			unset($_SESSION['category']);			
			unset($_SESSION['city']);			
        ...
        $query=$this->admin_model->Getallnewbypage($_SESSION['findkey'],$_SESSION['category'],$_SESSION['city'],$_SESSION['tinhot'],$_SESSION['tindo'],$_SESSION['tingap'],$_SESSION['tincao'],$start_row,$per_page);
		$total_rows =$query['total'] ;//$query->num_rows();				
		$this->load->library('pagination');
	...
	function baiviet()
    {
		...
		$query=$this->admin_model->gettbl_search_limited('tbl_baiviet','','');
		$total_rows = $query->num_rows();				
		$this->load->library('pagination');
		...
		$config['last_link'] = '>>';    		
		$this->pagination->initialize($config);    		
		$data['query']=$this->admin_model->gettbl_search_limited('tbl_baiviet',$_SESSION['start_row'],$per_page);			
		$data['pagination']= $this->pagination->create_links();	
    ...
    {        
		...
		$this->admin_model->add_tbl('tbl_baiviet',$data,$id);		 
		$this->sitemap(); 
	...
	function chuyenmuc()
    {
        ...
        $start_row=$this->uri->segment(3);
            ...
            $query=$this->admin_model->gettbl('tbl_chuyenmuc','');			
    		$total_rows = $query->num_rows();
    		$this->load->library('pagination');
    		...
    		$config['last_link'] = '>>';    		
    		$this->pagination->initialize($config);
    		$data['query']=$this->admin_model->gettbl_limited('tbl_chuyenmuc',$start_row,$per_page);
    		$data['pagination']= $this->pagination->create_links();	
	...
	function add_chuyenmuc()
    {
		...
		$this->admin_model->add_tbl('tbl_chuyenmuc',$data,$id);				
	...
	{     
        ...
        {            
            ...
            {
                $del_id = $checkbox[$i];					
				$result = $this->admin_model->del_tbl('tbl_chuyenmuc',$del_id);    				
			}
            if($result)
            {
                redirect('admin/chuyenmuc');        
